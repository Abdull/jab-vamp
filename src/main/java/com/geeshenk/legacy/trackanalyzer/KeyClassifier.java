...
import com.geeshenk.legacy.trackanalyzer.ApplicationParameters.key_t;
import com.geeshenk.legacy.trackanalyzer.ApplicationParameters.similarity_measure_t;
...
public class KeyClassifier {
	...
	public KeyClassifier(ApplicationParameters params) {
		// Profiles
		major = new ToneProfile(params.getToneProfile(), true, params);
		minor = new ToneProfile(params.getToneProfile(), false, params);
		silence = new ToneProfile(ApplicationParameters.tone_profile_t.TONE_PROFILE_SILENT, true, params);
	...
	public key_t classify(ArrayList<Float> chroma) {
		...
		if (similarityMeasure == ApplicationParameters.similarity_measure_t.SIMILARITY_CORRELATION) {
