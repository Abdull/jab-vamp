...
public class Chromagram {
    ...
    private void tuningHarte(ApplicationParameters params) throws Exception {
        /*
    ...
    private void tuningBinAdaptive(ApplicationParameters params) {
    ...
    public void reduceToOneOctave(ApplicationParameters params) {
        int oct = params.getOctaves();
        int bpo = bins / oct;
        if (bpo == bins) {
            return;
    ...
    public void reduceTuningBins(ApplicationParameters params) throws Exception {
        int oct = params.getOctaves();
        if (bins == 12 * oct) {
            return;
        }
        if (params.getTuningMethod() == ApplicationParameters.tuning_method_t.TUNING_BIN_ADAPTIVE) {
